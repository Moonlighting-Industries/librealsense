...
namespace librealsense
{
    ...
    public:
        ...
        void write_custom_header(const std::string& key, double value) override;
        void write_custom_header(const std::string& key, uint32_t value) override;
    ...
    private:
        ...
        void write_video_frame(const stream_identifier& stream_id, const nanoseconds& timestamp, frame_holder&& frame, bool write_metadata  = true);
        void write_motion_frame(const stream_identifier& stream_id, const nanoseconds& timestamp, frame_holder&& frame, bool write_metadata  = true);
        inline geometry_msgs::Vector3 to_vector3(const float3& f);
        inline geometry_msgs::Quaternion to_quaternion(const float4& f);
        void write_pose_frame(const stream_identifier& stream_id, const nanoseconds& timestamp, frame_holder&& frame, bool write_metadata  = true);
